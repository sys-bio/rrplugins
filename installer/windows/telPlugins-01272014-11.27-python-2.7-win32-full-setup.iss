...
#define LibRoadRunnerInstaller "libRoadRunner-1.0.0-minimal-setup.exe"
...
;#define LibRoadRunnerInstaller "http://sourceforge.net/projects/libroadrunner/files/libRoadRunner-1.0.0-minimal-setup.exe"
...
OutputDir=.
OutputBaseFilename=telplugins-01272014-11.27-python-2.7-win32-full-setup
SetupIconFile=Tellurium.ico
Compression=lzma
...
Source: "{#LibRoadRunnerInstaller}"; DestDir: "{tmp}"; Flags: ignoreversion
...
begin
 //itd_init;  //turned off when packaging installers
  ...
  ExtractTemporaryFile('{#LibRoadRunnerInstaller}');
...
begin
  ...
  //Lets install those files that were downloaded for us 
    ...
    if ForceDirectories(ExpandConstant('{app}')) then 
      ...
      RunOtherInstaller('{#LibRoadRunnerInstaller}');
      end;
    end;
end;