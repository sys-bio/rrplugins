...
{
    start_try
        PluginManager *pm = static_cast<PluginManager*>(castHandle(handle, typeid(PluginManager*).name(), __FUNC__));
        delete pm;
        return true;
    catch_bool_macro
}
...
bool tlp_cc unLoadPlugin(RRPluginManagerHandle handle, RRPluginHandle plugin)
{
    start_try
        PluginManager *pm = static_cast<PluginManager*>(castHandle(handle, typeid(PluginManager*).name(), __FUNC__));
        Plugin* aPlugin = castToPlugin(plugin, __FUNC__);
...
{
    start_try
        Plugin* aPlugin = castToPlugin(handle, __FUNC__);
