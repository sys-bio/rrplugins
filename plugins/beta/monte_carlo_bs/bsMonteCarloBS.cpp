...
MonteCarloBS::MonteCarloBS(PluginManager* manager)
:
...
bool MonteCarloBS::isWorking() const
{
    return mWorker.isRunning();
}

unsigned char* MonteCarloBS::getManualAsPDF() const
{
    return pdf_doc;
}

unsigned int MonteCarloBS::getPDFManualByteSize()
{
    return sizeofPDF;
}

StringList MonteCarloBS::getExperimentalDataSelectionList()
{
    return mExperimentalDataSelectionList.getValue();
}

string MonteCarloBS::getStatus()
{
    stringstream msg;
...
string MonteCarloBS::getImplementationLanguage()
{
    return ::getImplementationLanguage();
}

bool MonteCarloBS::resetPlugin()
{
...
string MonteCarloBS::getSBML()
{
    return mSBML.getValue();
}

string MonteCarloBS::getResult()
{
    stringstream msg;
...
bool MonteCarloBS::execute(bool inThread)
{
    try
    {
        Log(lInfo)<<"Executing the MonteCarloBS plugin";
...
MonteCarloBS* plugins_cc createPlugin(void* manager)
{
    //allocate a new object and return it
    return new MonteCarloBS((PluginManager*) manager);
}
...
void MonteCarloBS::assignPropertyDescriptions()
{
    stringstream s;
